...
verilator --lint-only -I"$RTL" "$RTL"/"$name"_ea.v # -I"$RTL" for multiple Verilog file dependencies in "src" folder

echo -e "${GREEN}IVerilog:------------------------------------------- ${NC}"
iverilog -g2005 -I"$RTL" "$RTL/"$name"_ea.v" "$SRC_FOLDER/${name}_tb.v" # -I"$RTL" for multiple Verilog file dependencies in "src" folder
